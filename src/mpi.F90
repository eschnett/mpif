module mpi
  ...
  ! Gather each address-kind specific from mpi_functions together with its
  ! TYPE(C_PTR) overload from mpi_cptr, which is the generic MPI-5.0 requires the
  ! mpi module to provide. The generics have to be declared here, where both
  ! halves are visible: one inside mpi_cptr would shadow the use-associated
  ...
  interface MPI_Win_allocate_c
     ...
     procedure MPI_Win_allocate_c_cptr
  ...
  interface MPI_Win_allocate_shared_c
     ...
     procedure MPI_Win_allocate_shared_c_cptr
  ...
  interface MPI_Win_shared_query_c
     ...
     procedure MPI_Win_shared_query_c_cptr
