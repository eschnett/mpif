# env DOCKER_BUILDKIT=1 docker build --file openmpi-llvm.dockerfile --platform linux/arm64 --progress plain --tag openmpi-mpiabi-llvm .
...
# Run tests
RUN ./build-openmpi-llvm-tests/version_f08
RUN ctest --test-dir build-openmpi-llvm-tests
