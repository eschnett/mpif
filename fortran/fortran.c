...
MPI_Comm PMPI_Comm_f2c(MPI_Fint comm) { return MPI_Comm_fromint(comm); }
MPI_Fint PMPI_Comm_c2f(MPI_Comm comm) { return MPI_Comm_toint(comm); }
MPI_Errhandler PMPI_Errhandler_f2c(MPI_Fint errhandler) { return MPI_Errhandler_fromint(errhandler); }
MPI_Fint PMPI_Errhandler_c2f(MPI_Errhandler errhandler) { return MPI_Errhandler_toint(errhandler); }
// not for MPICH: MPI_File PMPI_File_f2c(MPI_Fint file) { return MPI_File_fromint(file); }
// not for MPICH: MPI_Fint PMPI_File_c2f(MPI_File file) { return MPI_File_toint(file); }
MPI_Group PMPI_Group_f2c(MPI_Fint group) { return MPI_Group_fromint(group); }
MPI_Fint PMPI_Group_c2f(MPI_Group group) { return MPI_Group_toint(group); }
MPI_Info PMPI_Info_f2c(MPI_Fint info) { return MPI_Info_fromint(info); }
MPI_Fint PMPI_Info_c2f(MPI_Info info) { return MPI_Info_toint(info); }
MPI_Message PMPI_Message_f2c(MPI_Fint message) { return MPI_Message_fromint(message); }
MPI_Fint PMPI_Message_c2f(MPI_Message message) { return MPI_Message_toint(message); }
MPI_Op PMPI_Op_f2c(MPI_Fint op) { return MPI_Op_fromint(op); }
MPI_Fint PMPI_Op_c2f(MPI_Op op) { return MPI_Op_toint(op); }
MPI_Request PMPI_Request_f2c(MPI_Fint request) { return MPI_Request_fromint(request); }
MPI_Fint PMPI_Request_c2f(MPI_Request request) { return MPI_Request_toint(request); }
MPI_Session PMPI_Session_f2c(MPI_Fint session) { return MPI_Session_fromint(session); }
MPI_Fint PMPI_Session_c2f(MPI_Session session) { return MPI_Session_toint(session); }
MPI_Datatype PMPI_Type_f2c(MPI_Fint datatype) { return MPI_Type_fromint(datatype); }
MPI_Fint PMPI_Type_c2f(MPI_Datatype datatype) { return MPI_Type_toint(datatype); }
MPI_Win PMPI_Win_f2c(MPI_Fint win) { return MPI_Win_fromint(win); }
MPI_Fint PMPI_Win_c2f(MPI_Win win) { return MPI_Win_toint(win); }
...
// not for MPICH: MPI_File MPI_File_f2c(MPI_Fint file) { return PMPI_File_f2c(file); }
// not for MPICH: MPI_Fint MPI_File_c2f(MPI_File file) { return PMPI_File_c2f(file); }
