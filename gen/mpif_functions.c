...
{
  *ierror = PMPI_Abi_set_fortran_booleans(
    *logical_size,
    (void*)logical_true,
    (void*)logical_false
  );
...
{
  *ierror = PMPI_Abi_set_fortran_info(
    MPI_Info_fromint(*info)
  );
}
...
{
  const MPI_Aint result = PMPI_Aint_add(
    *base,
    *disp
  );
  return result;
...
{
  const MPI_Aint result = PMPI_Aint_diff(
    *addr1,
    *addr2
  );
  return result;
...
{
  *ierror = PMPI_Comm_call_errhandler(
    MPI_Comm_fromint(*comm),
    *errorcode
  );
}
...
)
{
  ...
  if (!mpif_predefined_callback((mpif_fortran_procedure)comm_delete_attr_fn, &c_comm_delete_attr_fn))
    c_comm_delete_attr_fn = mpif_attr_trampoline(MPIF_ATTR_COMM_DELETE);
  *ierror = PMPI_Comm_create_keyval(
    (MPI_Comm_copy_attr_function*)c_comm_copy_attr_fn,
    (MPI_Comm_delete_attr_function*)c_comm_delete_attr_fn,
    comm_keyval,
    (void*)*extra_state
...
{
  *ierror = PMPI_Comm_detach_buffer(
    MPI_Comm_fromint(*comm),
    buffer_addr,
    size
  );
...
{
  *ierror = PMPI_Comm_detach_buffer_c(
    MPI_Comm_fromint(*comm),
    buffer_addr,
    size
  );
...
{
  *ierror = PMPI_Comm_set_errhandler(
    MPI_Comm_fromint(*comm),
    MPI_Errhandler_fromint(*errhandler)
  );
}
...
{
  *ierror = PMPI_Dist_graph_neighbors(
    MPI_Comm_fromint(*comm),
    *maxindegree,
    sources,
    sourceweights,
...
{
  *ierror = PMPI_File_call_errhandler(
    MPI_File_fromint(*fh),
    *errorcode
  );
}
...
{
  *ierror = PMPI_File_get_byte_offset(
    MPI_File_fromint(*fh),
    *offset,
    disp
  );
...
{
  *ierror = PMPI_File_get_position_shared(
    MPI_File_fromint(*fh),
    offset
  );
}
...
{
  *ierror = PMPI_File_get_type_extent(
    MPI_File_fromint(*fh),
    MPI_Type_fromint(*datatype),
    extent
  );
...
{
  *ierror = PMPI_File_get_type_extent_c(
    MPI_File_fromint(*fh),
    MPI_Type_fromint(*datatype),
    extent
  );
...
{
  *ierror = PMPI_File_set_atomicity(
    MPI_File_fromint(*fh),
    mpif_logical2bool(*flag)
  );
}
...
{
  *ierror = PMPI_File_set_errhandler(
    MPI_File_fromint(*file),
    MPI_Errhandler_fromint(*errhandler)
  );
}
...
{
  *ierror = PMPI_Graph_neighbors_count(
    MPI_Comm_fromint(*comm),
    *rank,
    nneighbors
  );
...
{
  *ierror = PMPI_Group_translate_ranks(
    MPI_Group_fromint(*group1),
    *n,
    ranks1,
    MPI_Group_fromint(*group2),
...
)
{
  ...
  int c_buflen = 0;
  if (f_buflen > 0)
    c_buflen = (size_t)f_buflen <= length_value
                    ? (int)f_buflen + 1
                    : (int)length_value + 1;
  ...
  *ierror = MPI_Info_get_string(
  ...
  free(c_key);
  if (c_flag)
    *buflen = c_buflen > 0 ? (MPI_Fint)(c_buflen - 1) : 0;
  if (c_flag && f_buflen > 0)
    mpif_strcpy_c2f(value, c_value, length_value, strlen(c_value));
  *flag = mpif_bool2logical(c_flag);
}
...
{
  char* const c_datarep = mpif_strdup_f2c(datarep, length_datarep);
  *ierror = PMPI_Pack_external_size(
    c_datarep,
    *incount,
    MPI_Type_fromint(*datatype),
    size
...
{
  char* const c_datarep = mpif_strdup_f2c(datarep, length_datarep);
  *ierror = PMPI_Pack_external_size_c(
    c_datarep,
    *incount,
    MPI_Type_fromint(*datatype),
    size
...
)
{
  ...
  if (!mpif_predefined_callback((mpif_fortran_procedure)dtype_file_extent_fn, &c_dtype_file_extent_fn))
    c_dtype_file_extent_fn = (void*)mpif_datarep_extent_trampoline;
  *ierror = PMPI_Register_datarep_c(
    c_datarep,
    (MPI_Datarep_conversion_function_c*)c_read_conversion_fn,
    (MPI_Datarep_conversion_function_c*)c_write_conversion_fn,
    (MPI_Datarep_extent_function*)c_dtype_file_extent_fn,
...
{
  *ierror = PMPI_Remove_error_class(
    *errorclass
  );
}
...
{
  *ierror = PMPI_Remove_error_string(
    *errorcode
  );
}
...
{
  *ierror = PMPI_Session_call_errhandler(
    MPI_Session_fromint(*session),
    *errorcode
  );
}
...
{
  *ierror = PMPI_Session_detach_buffer(
    MPI_Session_fromint(*session),
    buffer_addr,
    size
  );
...
{
  *ierror = PMPI_Session_detach_buffer_c(
    MPI_Session_fromint(*session),
    buffer_addr,
    size
  );
...
{
  *ierror = PMPI_Session_flush_buffer(
    MPI_Session_fromint(*session)
  );
}
...
{
  *ierror = PMPI_Session_get_num_psets(
    MPI_Session_fromint(*session),
    MPI_Info_fromint(*info),
    npset_names
  );
...
{
  *ierror = PMPI_Session_set_errhandler(
    MPI_Session_fromint(*session),
    MPI_Errhandler_fromint(*errhandler)
  );
}
...
)
{
  ...
  if (!mpif_predefined_callback((mpif_fortran_procedure)type_delete_attr_fn, &c_type_delete_attr_fn))
    c_type_delete_attr_fn = mpif_attr_trampoline(MPIF_ATTR_TYPE_DELETE);
  *ierror = PMPI_Type_create_keyval(
    (MPI_Type_copy_attr_function*)c_type_copy_attr_fn,
    (MPI_Type_delete_attr_function*)c_type_delete_attr_fn,
    type_keyval,
    (void*)*extra_state
...
{
  *ierror = PMPI_Type_get_envelope_c(
    MPI_Type_fromint(*datatype),
    num_integers,
    num_addresses,
    num_large_counts,
...
{
  *ierror = PMPI_Type_get_true_extent(
    MPI_Type_fromint(*datatype),
    true_lb,
    true_extent
  );
...
{
  *ierror = PMPI_Type_get_true_extent_c(
    MPI_Type_fromint(*datatype),
    true_lb,
    true_extent
  );
...
{
  *ierror = PMPI_Type_get_true_extent_x(
    MPI_Type_fromint(*datatype),
    true_lb,
    true_extent
  );
...
{
  *ierror = PMPI_Win_call_errhandler(
    MPI_Win_fromint(*win),
    *errorcode
  );
}
...
{
  *ierror = PMPI_Win_flush_local_all(
    MPI_Win_fromint(*win)
  );
}
...
{
  *ierror = PMPI_Win_set_errhandler(
    MPI_Win_fromint(*win),
    MPI_Errhandler_fromint(*errhandler)
  );
}
...
{
  *ierror = PMPI_Win_shared_query_c(
    MPI_Win_fromint(*win),
    *rank,
    size,
    disp_unit,
...
{
  const double result = PMPI_Wtick(
  );
  return result;
}
...
{
  const double result = PMPI_Wtime(
  );
  return result;
}
